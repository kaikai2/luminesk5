...
void CreateRenderTexture(unsigned int textureArray[], int size, int channels, int type, int textureID)										
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, channels, size, size, 0, type, GL_UNSIGNED_SHORT, pTexture);						
...
void LuMotionBlur::init(ogl2d::Texture *_tex)
{
    tex = _tex;
}
...
void LuMotionBlur::render()
{
	...
	glPushMatrix();
      ...
      int viewport[2];
        glGetIntegerv(GL_MAX_VIEWPORT_DIMS, viewport);
        float hw = viewport[0]*.5f;
        float hh = viewport[1]*.5f;
    ...
    //*
		...
		// Stop drawing 
		glEnd();*/
		GLfloat pfTexCoord[] ={0.0f,viewport[1]/512.f,   0.0f,0.0f,  viewport[0]/512.f,0.0f,   viewport[2]/512.f, viewport[3]/512.f};
		glEnableClientState(GL_TEXTURE_COORD_ARRAY);
		glTexCoordPointer(2,GL_FLOAT,0,pfTexCoord);

		GLfloat pfVertices[] ={-hw,-hh,   -hw, hh,   hw, hh,   hw, -hh}; 
